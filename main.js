...
replayFileInput.addEventListener('change', async (e)=>{
  ...
  try{
    ...
    console.log("Replay loaded:", replayData.mapName, replayData.frames.length, "frames",
                "tickrate=", replayData.tickrate);
...
resetBtn.addEventListener('click',()=>{ playbackTime=0; });
...
function animate(){
  ...
  if(replayData && isPlaying){
    ...
    if(index>= maxIndex){
      index= maxIndex-1;
      playbackTime= index*frameTime; // freeze at last
    }
    const next= index+1;
    ...
    // ALWAYS FLIP FORWARD:
    // ex. camera.x= x+zx, camera.y= z, camera.z= -(y+zy)
